...
class Pin(object):
    def __init__(self, sku=None, sid='', nucleus_id=0, persona_id='', dob=False, platform=False):
        ...
        self.plat = None #re.search('plat:"(.+?)"', rc).group(1)
        ...
        self.taxv = None#re.search('PinManager.TAXONOMY_VERSION=([0-9\.]+)', rc).group(1)
        self.tidt = None#re.search(',PinManager.TITLE_ID_TYPE="(.+?)"', rc).group(1)
